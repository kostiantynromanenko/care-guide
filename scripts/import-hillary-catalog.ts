...
/**
 * One-time (re-runnable) import of real HiLLARY catalog data, replacing
 * fictional demo-content placeholders per the "work with real data" decision
 * (docs/PROJECT_CONTEXT.md). Idempotent: re-running updates existing docs by
 * slug instead of duplicating them.
 *
 * Scope (see docs/PROJECT_CONTEXT.md for the full reasoning):
...
async function loadOffers(): Promise<Map<string, FeedOffer>> {
  ...
  if (fs.existsSync(CACHE_PATH)) {
    ...
    xml = fs.readFileSync(CACHE_PATH, "utf8");
  } else {
...
async function run() {
  ...
  console.log(`Loaded ${offers.size} offers from feed.`);
  ...
  for (const mapping of allMappings) {
    ...
    if (!offer) {
      console.warn(`Offer ${mapping.offerId} not found in feed, skipping ${mapping.slug}`);
      continue;
    }
    ...
    if (pictureUrl) {
      try {
        ...
        console.warn(`Failed to fetch/upload image for ${mapping.slug}:`, error);
      }
    }
    ...
    if (existing.docs.length > 0) {
      ...
      console.log(`Updated product ${mapping.slug} <- offer ${mapping.offerId}`);
    } else {
      const created = await payload.create({
        collection: "products",
      ...
      console.log(`Created product ${mapping.slug} <- offer ${mapping.offerId}`);
    }
  }
  ...
  for (const mapping of NEW_COLLECTIONS) {
    ...
    if (existingId) {
      ...
      console.log(`Updated collection ${mapping.slug}`);
    } else {
      const created = await payload.create({
      ...
      console.log(`Created collection ${mapping.slug}`);
    }
  }
