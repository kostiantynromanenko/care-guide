...
/**
 * Loads `demo/demo-content.json` into Payload's collections/globals.
 * Shared by `scripts/seed.ts` (local dev, run against the local Docker
 * Postgres) and the protected `/api/seed` route (production, run against
 * whichever Postgres is configured — see docs/DEPLOYMENT.md § "Seed initial
 * content", since production secrets can't be read back to run this locally).
 *
 * Not idempotent by design: unique slug constraints will throw on a second
 * run, which is an intentional guard against accidentally double-seeding.
 */
export async function seedDemoContent(payload: Payload) {
  ...
  return {
    needs: demoContent.needs.length,
    products: productSlugToId.size,
    collections: collectionSlugToId.size,
